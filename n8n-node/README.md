...
| `MCP_AUTH_EMAIL` | Your GoCertius account email. Configure exactly one flow; do not combine it with the others. | no |
| `MCP_AUTH_PASSWORD` | Your GoCertius account password. (See https://www.gocertius.io for credential acquisition.) | yes |
| `MCP_AUTH_USER_KEY` | Long-lived GoCertius user key, exchanged automatically for a short-lived session token. Use it for headless or automated access instead of an account password. Configure exactly one flow; do not combine it with the others. (See https://www.gocertius.io for credential acquisition.) | yes |
